...
#ifdef PICO_RP2350
    if (used_sm < 0 || !pio_can_add_program(used_pio, &tandy_program)) { // If no free sm or memory on PIO1, try PIO2 (not on Pico1)
        used_pio = pio2;
        used_sm = pio_claim_unused_sm(used_pio, false);
    }
#endif
...
bool load_tandy(Device *self) {
    choose_sm();
    if (used_sm < 0 || !pio_can_add_program(used_pio, &tandy_program)) { // If not any PIO with free sm and enough memory, abort
        return false;
    }

    used_offset = pio_add_program(used_pio, &tandy_program);
    if (used_offset < 0) {
        return false;
    }
...
bool unload_tandy(Device *self) {
    ...
    pio_remove_program_and_unclaim_sm(&tandy_program, used_pio, used_sm, used_offset);
...
size_t generate_tandy(Device *self, int16_t *left_sample, int16_t *right_sample) {
    ...
    *right_sample = current_sample;
    ...
    // write_register(uint32_t address, uint8_t data); <- address is always 0xc0 (not checked)
}
